...
namespace SEB_NAMESPACE {
  ...
  template<typename Float, class Pt, class PointAccessor>
  std::array< double, 4 > Smallest_enclosing_ball<Float, Pt, PointAccessor>::verify()
  {
    using std::inner_product;
    using std::abs;
    using std::cout;
    ...
    for (unsigned int k = 0; k < S.size(); ++k) {
      ...
      ball_error = sqrt(inner_product(center_to_point,center_to_point+dim,
                                      center_to_point,Float(0))) - radius_;
    ...
    output[0] = min_lambda; 
    output[1] = (max_overlength / radius_);
    output[2] = (abs(min_underlength / radius_));
    output[3] = qr_error; 
    return(output);
  }
